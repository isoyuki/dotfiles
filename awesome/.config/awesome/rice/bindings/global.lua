...
local global_bindings = {
	...
	binding.new({
		...
		on_press = function()
			awful.spawn(config.actions.show_launcher)
		end,
	}),
...
if config.features.screenshot_tools then
	global_bindings = {
		...
		binding.new({
			...
			on_press = function()
				awful.spawn.with_shell('flameshot gui --region "$(slop -q -c 1,0,0,0.5 -b 5 -n 0 -l -t 9999999)"')
			end,
		}),
		...
		binding.new({
			...
			on_press = function()
				awful.spawn.with_shell(
					"flameshot gui --region \"$(printf '%s' \"$DISPLAY\" | sd '^:(\\d+)' 'screen$1')\""
				)
			end,
		}),
		...
		binding.new({
			...
			on_press = function()
				awful.spawn.with_shell("flameshot gui --region all")
			end,
		}),
