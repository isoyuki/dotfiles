...
local function update_player(self, player_data)
	...
	if self.player_container.visible then
		...
		local button_style = player_data and beautiful.capsule.styles.normal or beautiful.capsule.styles.disabled
...
local function update_playback_status(self, player_data)
	...
	self._private.content_container:apply_style(
		is_playing and beautiful.media_player.content_styles.normal or beautiful.media_player.content_styles.disabled
	)

	self
		._private
		.play_pause_button
		.widget --[[@as wibox.widget.imagebox]]
		:set_image(is_playing and beautiful.icon("pause.svg") or beautiful.icon("play.svg"))
...
local function initialize_content_container(self)
	...
	self._private.drag_seeking_interrupt = select(
		2,
	...
	self._private.wheel_seeking_interrupt = select(
		2,
		hmouse.attach_wheel({
			widget = self,
